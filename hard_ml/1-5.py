...
class KNRM(torch.nn.Module):
    ...
    def _get_kernels_layers(self) -> torch.nn.ModuleList:
        ...
        if self.kernel_num > 1:
            ...
            mu_list.append(1 - step / 2)
            for i in range(int(self.kernel_num) - 2):
                mu_list.append(mu_list[-1] - step)
            mu_list = list(reversed(mu_list))
            # mu_list = np.array(mu_list)
        sigmas = [self.sigma] * (self.kernel_num - 1) + [self.exact_sigma]

        for i in range(self.kernel_num):
            kernels.append(GaussianKernel(mu_list[i], sigmas[i]))
        return kernels
        ...
        if self.out_layers:
            ...
            if len(hidden_sizes) > 2:
                for i in range(len(hidden_sizes) - 2):
                    output.append(torch.nn.Linear(hidden_sizes[i], hidden_sizes[i + 1]))
                    output.append(torch.nn.ReLU())
            output.append(torch.nn.Linear(hidden_sizes[-2], hidden_sizes[-1]))
        else:
...
class Solution:
    ...
    def hadle_punctuation(self, inp_str: str) -> str:
        return ''.join(ch if ch not in set(string.punctuation) else ' ' for ch in inp_str)

    def simple_preproc(self, inp_str: str) -> List[str]:
        current = self.hadle_punctuation(inp_str)
        return word_tokenize(current.lower())

    def _filter_rare_words(self, vocab: Dict[str, int], min_occurancies: int) -> Dict[str, int]:
        tokens_to_remove = []
        for token, qnt in vocab.items():
            if qnt < min_occurancies:
    ...
    def get_all_tokens(self, list_of_df: List[pd.DataFrame], min_occurancies: int) -> List[str]:
        ...
        texts = list(df['text_left']) + list(df['text_right'])
    ...
    def create_glove_emb_from_file(self, file_path: str, inner_keys: List[str],
        ...
        glove_vocab = self._read_glove_embeddings(file_path)
        inner_keys = ['PAD'] + ['OOV'] + inner_keys
        vocab = {}
        emb_matrix = []
        unk_words = []
        for idx, token in enumerate(inner_keys):
            if token in glove_vocab.keys():
                emb_matrix.append(np.array(glove_vocab[token], dtype=np.float32))
            elif token == 'PAD':
                emb_matrix.append(np.zeros(50))
                unk_words.append(token)
            else:
                emb_matrix.append(np.random.uniform(-rand_uni_bound, rand_uni_bound, 50))
                unk_words.append(token)
            vocab[token] = idx
